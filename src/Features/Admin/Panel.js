...
const displayMembersForGroup = (members, sendEmailToMember) => {
  const memberRows = members.map(
    ({ memberName, email, drawn, wishlistUpdated, lastLoggedIn }) => (
      <Table.Row key={memberName} data-testid={`row-${memberName}`}>
        ...
        <Table.Cell className="format-center">
          {drawn ? (
            <Icon
              name="check"
              color="green"
              data-testid={`${memberName}-giftee-assigned`}
            />
          ) : (
            <Icon
              name="close"
            ...
            <Popup
              trigger={
                <Icon
                  name="check"
                  color="green"
                  data-testid={`${memberName}-wishlist-updated`}
                />
              }
        ...
        <Table.Cell data-testid={`${memberName}-last-logged-in`}>
          {lastLoggedIn ? formatDate(lastLoggedIn) : 'never'}
        </Table.Cell>
        <Table.Cell data-testid={`${memberName}-send-email`}>
          <Button
            color="pink"
            type="button"
            id={memberName}
  ...
  return (
    <Table celled className="groups-admin" id="membersList">
      ...
      <Table.Body>{memberRows}</Table.Body>
    </Table>
  );
};
...
const allGroups = (groups, handleDraw, deleteGroup, sendEmailToAll) => {
  const groupRows = groups.map(({ groupName, count }) => (
    <Table.Row key={groupName} data-testid={`row-${groupName}`}>
      ...
      <Table.Cell data-testid={`count-${groupName}`} singleLine>
        {count}
      </Table.Cell>
      <Table.Cell>
        <Button
          ...
          id={groupName}
          onClick={handleDraw}
          data-testid={`${groupName}-draw-btn`}
        >
          Draw
        </Button>
      </Table.Cell>
      <Table.Cell>
        <Button
          ...
          id={groupName}
          onClick={deleteGroup}
          data-testid={`${groupName}-delete-btn`}
        >
          Delete
        </Button>
      </Table.Cell>
      <Table.Cell>
        <Button
          ...
          data-testid={`${groupName}-send-email-all-btn`}
        >
          Send
        </Button>
      </Table.Cell>
...
export default function Panel() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const { data } = await api.get('/admin/allgroups', {
  ...
  const sendEmailToMember = async (event) => {
    ...
    const token = getToken();
    await api
      .get(`/admin/sendEmail/${searchGroupName}/${memberName}`, {
        headers: setAuthorisationToken(token),
      })
      .then(() =>
        displayNotification({
  ...
  const handleSetupGroup = async (event) => {
    event.preventDefault();
    const payload = newMembers.reduce(
      (acc, { memberName, email }) => [
        ...acc,
        { memberName: memberName.toLowerCase(), email: email.toLowerCase(0) },
      ],
      []
    );
    ...
    if (groupNameValue) {
      ...
      await api
        .post(`/admin/setup/${groupName}`, payload, {
          headers: setAuthorisationToken(token),
        })
        .then(() => {
          displayNotification({
  ...
  const deleteGroup = async (event) => {
    ...
    const { data } = await api
      .delete(`/admin/${groupName}`, { headers: setAuthorisationToken(token) })
      .catch((err) =>
        displayNotification({
          type: 'negative',
  ...
  const handleSearchSubmit = async (event) => {
    ...
    if (searchGroupName) {
      ...
      await api
        .get(`/admin/${groupName}`, { headers: setAuthorisationToken(token) })
        .then(({ data }) => {
